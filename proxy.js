...
function onconnect(req, socket, head) {
	...
	authenticate(this, req, function(err, auth) {
		...
		debug.proxyRequest('method %j', req.method);

		let statusCode = 200;
		const chaosPercentage = 70;
		const chance = Math.floor(Math.random() * 100);
		debug.proxyRequest('chance %j < %j', chance, chaosPercentage);
		if (chance < chaosPercentage) {
			statusCode = getRandomStatusCode('GET');
		}
		debug.proxyRequest('status code %j', statusCode);
...
function getRandomStatusCode(requestMethod) {
